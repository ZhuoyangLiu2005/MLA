...
from util import check_bloat16_supported, SplitModalitySampler, PaddedCollatorForActionPrediction, PaddedCollatorForLanguageModeling
...
class TrainingStrategy(ABC):
    def __init__(
        ...
        **_: str,
        ...
        self.grad_accumulation_steps = self.global_batch_size // self.per_device_batch_size // overwatch.world_size() # 注意更改！！！
            ...
            for train_idx, batch in enumerate(dataloader):
                ...
                with torch.autocast(
                    ...
                    if use_diff:
                        loss_dict, output = self.vlm(
                            ...
                            images=batch["images"],
                            next_images=batch["next_images"] if (use_generation and gen_image) else None,
                            camera_name=camera_name,
                            point_cloud=batch["point_cloud"] if use_pointcloud else None,
                            next_point_cloud=batch["next_point_cloud"] if (use_pointcloud and use_generation and gen_pointcloud) else None,
                            tactile=batch["tactile"] if use_tactile else None,
                            next_tactile=batch["next_tactile"] if (use_tactile and use_generation and gen_tactile) else None,
                        ...
                        metrics.commit(loss=loss_dict['total_loss'],
                                    img_pc_contrastive_loss=loss_dict['img_pc_contrastive_loss'],
                                    tactile_contrastive_loss=loss_dict['tactile_contrastive_loss'],
                                    diff_loss=loss_dict['diff_loss'],
                                    image_gen_loss=loss_dict['image_gen_loss'],
                                    point_cloud_gen_loss=loss_dict['point_cloud_gen_loss'],
                                    tactile_gen_loss=loss_dict['tactile_gen_loss'],
                                ) 
                        ...
                        loss_dict, output = self.vlm(
                            ...
                            attention_mask=batch["attention_mask"],
                            images=batch["images"],
                            next_images=batch["next_images"] if use_generation else None,
                            camera_name=camera_name,
                            point_cloud=batch["point_cloud"] if use_pointcloud else None,
                            next_point_cloud=batch["next_point_cloud"] if (use_pointcloud and use_generation) else None,
                            tactile=batch["tactile"] if use_tactile else None,
                            next_tactile=batch["next_tactile"] if (use_tactile and use_generation and gen_tactile) else None,
                            proprio=batch["proprio"],
                            labels=batch["labels"],
                ...
                if (train_idx + 1) % self.grad_accumulation_steps == 0:
                    ...
                    if (self.max_steps is not None and metrics.global_step >= self.max_steps) or (
                        (metrics.global_step % (math.ceil((len(dataloader) / overwatch.world_size() / self.grad_accumulation_steps)))) == 0 and epoch % save_interval == 0
                    ):
                        # terminate=True
