...
def make_dataset_from_rlds(
    ...
    *,
    ...
    load_tactile: Optional[str] = None,
    ...
    def restructure(traj):
        ...
        if load_tactile is not None:
            new_obs["gripper_xyz"] = tf.cast(traj["observation"]["gripper_xyz"], tf.float32)
            new_obs["tactile_right"] = tf.cast(traj["observation"]["tactile_right"], tf.float32)
            new_obs["tactile_left" ] = tf.cast(traj["observation"]["tactile_left"], tf.float32)
            new_obs["next_tactile_right"] = tf.cast(traj["observation"]["next_tactile_right"], tf.float32)
            new_obs["next_tactile_left"] = tf.cast(traj["observation"]["next_tactile_right"], tf.float32)
