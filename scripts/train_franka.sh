...
SETTING=Pretrain${PRETRAIN}_FreezeVis${FREEZE_VISON}_Window${FUTURE_ACTION_STEPS}_Diff${USE_DIFF}_Rec${USE_REC}_Contrastive${USE_CONTRASTIVE}_Vislayer${LLM_VISION_LAYERS}_1024_0403_0825

TASK=wipe_whiteboard_0824_keyframe
BATCH_SIZE=8
EPOCHS=300
LEARNING_RATE=2e-5
...
NODES=3
MASTER_ADDR="10.200.64.82" # ifconfig
NODE_RANK=2

DATA_ROOT=/media/liuzhuoyang/data/franka/rlds
...
torchrun --nnodes $NODES --nproc-per-node $NUM_GPUS --node_rank=$NODE_RANK --master_addr=${MASTER_ADDR} --master_port=29501 scripts/train.py \
  ...
  --is_resume False \
  --pretrained_checkpoint "/media/liuzhuoyang/new_vla/Rec_Diff_beta/pretrain-exp/exp_rtx_0812_Pretrainopenvla_FreezeVisfalse_Window0_Difftrue_Recfalse2d_Contrastive_Vislayer8_1024_0403_0818/checkpoints/step-028396-epoch-01-loss=0.0514.pt"
