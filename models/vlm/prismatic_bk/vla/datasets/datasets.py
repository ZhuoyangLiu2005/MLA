...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "image_wrist_right" in rlds_batch["observation"]:
            wrist_right_image = Image.fromarray(rlds_batch["observation"]["image_wrist_right"][0])
            wrist_right_image = self.image_transform.preprocess(wrist_right_image, return_tensors='pt')['pixel_values'][0]   
        wrist_left_image = None
        if "image_wrist_left" in rlds_batch["observation"]:
            wrist_left_image = Image.fromarray(rlds_batch["observation"]["image_wrist_left"][0])
            wrist_left_image = self.image_transform.preprocess(wrist_left_image, return_tensors='pt')['pixel_values'][0]   
        ...
        if wrist_right_image is not None:
            wrist_right_image = torch.cat([wrist_right_image, image_mask], dim=0)
            rgb_images['wrist_right_image']=wrist_right_image
        if wrist_left_image is not None:
            wrist_left_image = torch.cat([wrist_left_image, image_mask], dim=0)
            rgb_images['wrist_left_image']=wrist_left_image
            
        # load tactile_vectors if needed
        if self.use_tactile:
            tactile_right=rlds_batch["observation"]['tactile_right'][0]
            tactile_right = torch.tensor(tactile_right, dtype=torch.float32)
            tactile_left=rlds_batch["observation"]['tactile_left'][0]
            tactile_left = torch.tensor(tactile_left, dtype=torch.float32)
            tactile_right = torch.where(tactile_right == 65535, torch.tensor(0.0, device=tactile_right.device), tactile_right)
            tactile_left = torch.where(tactile_left == 65535, torch.tensor(0.0, device=tactile_left.device), tactile_left)
            tactile = torch.cat([tactile_right, tactile_left], dim=0)
            tactile = tactile / 100.0
            # print(tactile.shape)
            # input()
            next_tactile_right=rlds_batch["observation"]['next_tactile_right'][0]
            next_tactile_right = torch.tensor(next_tactile_right, dtype=torch.float32)
            next_tactile_left=rlds_batch["observation"]['next_tactile_left'][0]
            next_tactile_left = torch.tensor(next_tactile_left, dtype=torch.float32)
            next_tactile_right = torch.where(next_tactile_right == 65535, torch.tensor(0.0, device=next_tactile_right.device), next_tactile_right)
            next_tactile_left = torch.where(next_tactile_left == 65535, torch.tensor(0.0, device=next_tactile_left.device), next_tactile_left)
            next_tactile = torch.cat([next_tactile_right, next_tactile_left], dim=0)
            next_tactile = next_tactile / 100.0
