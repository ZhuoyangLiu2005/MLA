...
from models.backbones import LLMBackbone
# from models.vlm.prismatic.models.backbones.vision import VisionBackbone
from models.vlm import PrismaticVLM
from models.mla.image import VisionTokenizer, MLP_GELU
from models.mla.pointcloud import PointTokenizer
...
class MLA(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.gen_pointcloud = gen_pointcloud
        self.gen_tactile = gen_tactile
        self.vlm = vlm
    ...
    def forward(
        ...
        if self.use_diff:
            ...
            if self.use_generation and self.gen_image:
            ...
            if self.use_pointcloud and self.use_generation and self.gen_pointcloud:
                next_point_cloud = next_point_cloud.repeat(repeated_diffusion_steps, *([1] * (next_point_cloud.ndimension() - 1)))
            if self.use_tactile:
                tactile = tactile.repeat(repeated_diffusion_steps, *([1] * (tactile.ndimension() - 1)))
                gripper_xyz = gripper_xyz.repeat(repeated_diffusion_steps, *([1] * (gripper_xyz.ndimension() - 1)))
            if self.use_tactile and self.use_generation and self.gen_tactile:
                next_tactile = next_tactile.repeat(repeated_diffusion_steps, *([1] * (next_tactile.ndimension() - 1)))
            ...
            output, noise_pred, generation_outputs, generation_losses = self.vlm(
                input_ids=input_ids,
            ...
            loss_dict = {
                ...
                'img_pc_contrastive_loss': torch.tensor(0, dtype=torch.float32),
                'tactile_contrastive_loss': torch.tensor(0, dtype=torch.float32),
                'diff_loss': torch.tensor(0, dtype=torch.float32),
                'image_gen_loss': torch.tensor(0, dtype=torch.float32),
                'point_cloud_gen_loss': torch.tensor(0, dtype=torch.float32),
                'tactile_gen_loss': torch.tensor(0, dtype=torch.float32),
            }
            diff_loss = ((noise_pred - noise) ** 2).mean()
            loss_dict['total_loss'] = diff_loss
            loss_dict['diff_loss'] = diff_loss
            if self.use_generation and self.gen_image:
                loss_dict['image_gen_loss'] = generation_losses['image_gen_loss']
                loss_dict['total_loss'] += generation_losses['image_gen_loss']
            if self.use_generation and self.gen_pointcloud:
                loss_dict['point_cloud_gen_loss'] = generation_losses['point_cloud_gen_loss']
                loss_dict['total_loss'] += generation_losses['point_cloud_gen_loss']
            if self.use_generation and self.gen_tactile:
                loss_dict['tactile_gen_loss'] = generation_losses['tactile_gen_loss']
                loss_dict['total_loss'] += generation_losses['tactile_gen_loss']
            if self.use_contrastive:
                loss_dict['img_pc_contrastive_loss'] = output.img_pc_contrastive_loss
                loss_dict['total_loss'] += output.img_pc_contrastive_loss
                if self.use_tactile:
                    loss_dict['tactile_contrastive_loss'] = output.tactile_contrastive_loss
                    loss_dict['total_loss'] += output.tactile_contrastive_loss
            print(loss_dict)
            return loss_dict, output
        else:
            output, generation_outputs, generation_losses = self.vlm(
                input_ids=input_ids,
                attention_mask=attention_mask,
                images=images,
                next_images=next_images,
            ...
            loss_dict = {
                ...
                'ar_loss': torch.tensor(0, dtype=torch.float32),
                'image_gen_loss': torch.tensor(0, dtype=torch.float32),
                'point_cloud_gen_loss': torch.tensor(0, dtype=torch.float32),
                'tactile_gen_loss': torch.tensor(0, dtype=torch.float32),
            }
            loss_dict["total_loss"] = output.loss
            loss_dict["ar_loss"] = output.loss
            if self.use_generation and self.gen_image:
                loss_dict['image_gen_loss'] = generation_losses['image_gen_loss']
                loss_dict['total_loss'] += generation_losses['image_gen_loss']
            if self.use_generation and self.gen_pointcloud:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if "proprio_embedder" in model_state_dict.keys() and model_state_dict["proprio_embedder"]["mlp.fc1.weight"].shape[-1] == action_dim:
        ...
        # Try to load tactile_embedder
        if action_dim == 14:
            tactile_dim = 24
        elif action_dim == 7:
            tactile_dim = 12
        if use_tactile and "tactile_embedder" in model_state_dict.keys() and model_state_dict["tactile_embedder"]["mlp.fc1.weight"].shape[-1] == tactile_dim:
        ...
        if use_diff and "x_embedder" in model_state_dict.keys() and "t_embedder" in model_state_dict.keys() and "final_layer" in model_state_dict.keys():
            ...
            print("\n\nSuccessfully loaded x_embedder, t_embedder, final_layer from checkpoint!!!!\n")
        else:
            print("\n\nNo x_embedder, t_embedder, final_layer found in checkpoint!!!!\n")

        if use_generation and "generation_manager" in model_state_dict.keys() and gen_image:
            gen_state_dict = {}
            for k, v in model_state_dict["generation_manager"].items():
                if k.startswith("image_gen_module."):
                    new_key = k[len("image_gen_module."):]
                    gen_state_dict[new_key] = v
            if gen_state_dict:
                vlm.generation_manager.image_gen_module.load_state_dict(gen_state_dict)
                print("\n\nSuccessfully loaded generation_manager.image_gen_module from checkpoint!!!!\n")
            else:
                print("\n\nNo generation_manager.image_gen_module found in checkpoint!!!!\n")
        else:
            print("\n\nNo generation_manager.image_gen_module found in checkpoint!!!!\n")

        if use_generation and "generation_manager" in model_state_dict.keys() and gen_pointcloud:
            gen_state_dict = {}
            for k, v in model_state_dict["generation_manager"].items():
                if k.startswith("pointcloud_gen_module."):
                    new_key = k[len("pointcloud_gen_module."):]
                    gen_state_dict[new_key] = v
            if gen_state_dict:
                vlm.generation_manager.pointcloud_gen_module.load_state_dict(gen_state_dict)
                print("\n\nSuccessfully loaded generation_manager.pointcloud_gen_module from checkpoint!!!!\n")
            else:
                print("\n\nNo generation_manager.pointcloud_gen_module found in checkpoint!!!!\n")
        else:
            print("\n\nNo generation_manager.pointcloud_gen_module found in checkpoint!!!!\n")

        if use_generation and "generation_manager" in model_state_dict.keys() and gen_tactile:
            gen_state_dict = {}
            for k, v in model_state_dict["generation_manager"].items():
                if k.startswith("tactile_gen_module."):
                    new_key = k[len("tactile_gen_module."):]
                    gen_state_dict[new_key] = v
            if gen_state_dict:
                vlm.generation_manager.tactile_gen_module.load_state_dict(gen_state_dict)
                print("\n\nSuccessfully loaded generation_manager.tactile_gen_module from checkpoint!!!!\n")
            else:
                print("\n\nNo generation_manager.tactile_gen_module found in checkpoint!!!!\n")
        else:
            print("\n\nNo generation_manager.tactile_gen_module found in checkpoint!!!!\n")
